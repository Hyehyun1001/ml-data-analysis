...
전처리 이전에 확인해야 할, "정보량이 없어 제거 대상이 되는 컬럼" 두 가지를 한 절로 정리한다. 실제 제거는 둘 다 4.4절(피처 정제)에서 함께 수행한다.

**동일 값(상수) 컬럼** — 전체 369개 피처 중 **34개 컬럼**은 모든 76,020개 샘플에서 값이 단 하나뿐인 상수(constant) 컬럼이었다(예: 특정 상품을 가진 고객이 데이터셋 안에 아예 없어 해당 지표가 항상 0인 경우). 상수 컬럼은 분산이 0이므로 어떤 모델을 쓰든 예측에 전혀 기여할 수 없다.

**중복 컬럼** — 상수 컬럼을 먼저 제거한 뒤 남은 335개 컬럼을 대상으로 전치(`.T`) 후 `duplicated()`로 확인한 결과, **29개 컬럼**이 다른 컬럼과 76,020개 값이 완전히 동일한 중복 컬럼이었다(예: `ind_var26_0`과 `ind_var26`, `num_var26_0`과 `num_var26`처럼 `_0` 접미사가 붙은 버전과 안 붙은 버전이 사실상 같은 값을 가지는 경우가 다수). 중복 컬럼을 남겨두면 다중공선성만 키우고 정보량은 늘지 않는다.
...
2. **중복 컬럼 제거**: 다른 컬럼과 완전히 동일한 값을 가지는 컬럼 29개 제거 (2.4절, 다중공선성 방지).
...
팀 노트북 10개를 전수 조사한 결과, 명시적으로 `def`로 정의된 재사용 함수는 예상보다 적었다 — 혜현(GBM)·진희(XGBoost)·용우(LightGBM)가 각자 독립적으로 작성한 Hyperopt `objective(params)` 함수, 그리고 유경이 로지스틱 회귀를 위해 작성한 `fit_woe`/`transform_woe` 두 함수 정도가 전부였다. 나머지 이상치 치환·파생 변수 생성·모델 학습/평가 코드는 대부분 노트북 셀에 인라인으로 반복 작성되었다. 다만 예외적으로 **`train_test_split`을 두 번 적용해 학습(80%)·검증(학습 세트의 30%)·테스트(20%)로 나누고, 두 번 모두 `stratify=`를 지정해 클래스 비율(96:4)을 유지하는 2단계 분할 방식**만큼은 어느 노트북에서도 함수로 따로 빼두지 않았음에도 거의 모든 노트북에서 그대로 반복되었다 — 데이터 누수 방지와 클래스 불균형 보정이라는 원칙만큼은 명문화된 함수 없이도 팀 전체에 잘 공유되어 있었다는 뜻이다. 즉 **"공동 함수"라기보다는 "유사한 패턴을 각자 독립적으로 재구현"한 경우가 대부분**이었다는 것이 정직한 실태이며, 이는 회고(10장)의 개선점("공통 전처리 함수를 팀 초기에 모듈화")과 직접 연결된다. 실제로 반복된 패턴들을 정리해 재사용 가능한 형태로 다듬은 코드는 12장 부록에 정리했다.
...
| 모델 비교 논리와 지표 선정의 적절성 | 클래스 불균형을 근거로 ROC-AUC를 공식 지표로 통일했고(2.6절), 6개 모델을 동일 기준(홀드아웃 테스트 세트)으로 비교했다. 다만 7.1절에서 밝혔듯 팀원별 "튜닝 전" 기준 파이프라인이 서로 달라 절대 비교보다 개선폭 비교에 그친 한계가 있다. |
...
def remove_useless_columns(X_features):
    """상수 컬럼 + 중복 컬럼(2.4절) 제거. 반드시 상수 컬럼을 먼저 제거한 뒤
